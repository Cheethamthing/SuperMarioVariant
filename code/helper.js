...
// Contains most of the helper functions that keeps the game running smoothly
...
function createActorElement(actor, stateStatus) {
...
export function drawActors(state) {
    ...
    return elt(
        "div",
        {},
        ...actors.map(actor => createActorElement(actor, state.status))
    );
}
